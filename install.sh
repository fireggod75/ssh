...
echo '<Directory /var/www/html/firegod/>
...
chown www-data:www-data /var/www/html/firegod/* &
wait
...
chown apache:apache /var/www/html/firegod/* &
wait


sudo sed -i "s/apache2/httpd/g" /var/www/html/firegod/setting.php &
wait
...
sudo sed -i "s/22/$port/g" /var/www/html/firegod/config.php &
wait 
sudo sed -i "s/adminuser/$adminusername/g" /var/www/html/firegod/config.php &
wait 
sudo sed -i "s/adminpass/$adminpassword/g" /var/www/html/firegod/config.php &
wait 
curl -u "$adminusername:$adminpassword" "http://${ipv4}/firegod/restoretarikh.php"

cp /var/www/html/firegod/tarikh /var/www/html/firegod/backup/tarikh

rm -fr /var/www/html/firegod/tarikh
echo "5 0 * * * php /var/www/html/firegod/expire.php >/dev/null 2>&1" | crontab - &
wait

clear
printf "\nPanel Link : http://${ipv4}/firegod/index.php"
